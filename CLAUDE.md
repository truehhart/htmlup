...
- **`htmlup` does no lifecycle management of uploaded files.** It uploads and exits. (GitHub Pages cleanup is delegated to opt-in cron GitHub Actions in the *target* repo — see ARCHITECTURE.) S3 lifecycle is the bucket owner's problem.
...
- **All scripts are Nushell** (`mise-tasks/` with `#!/usr/bin/env nu`). Validate with `nu-check` after editing.
