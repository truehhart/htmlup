...
- **The publish path does no lifecycle management of uploaded files.** It uploads and exits — it never deletes. GitHub Pages cleanup is delegated to an opt-in cron GitHub Actions workflow in the *target* repo; `htmlup setup github` installs that workflow once (it does the deleting, on a schedule, in the target repo — see ARCHITECTURE). S3 lifecycle is the bucket owner's problem.
...
| `internal/provider/github/` | GitHub Pages backend (`go-github`); includes `setup github` (`setup.go`) to bootstrap a target repo |
