...
GoReleaser produces static binaries for `linux`/`darwin` × `amd64`/`arm64` (no Windows). Releases are cut by the **manually-triggered `release.yaml`** workflow: it takes a `version` input (without the leading `v`), validates it, and runs the check suite. Only then does publish **create and push the `v<version>` tag and check it out**, so the artifacts are built *from the tag*, not from whatever the branch happens to be. GoReleaser builds + signs the archives and `SHA256SUMS` (cosign keyless + GPG), and `softprops/action-gh-release` creates the release from that tag, generates notes, and uploads the signed artifacts (updating an existing release gracefully). Pushing the tag only after checks pass avoids leaving an orphan tag behind a failed release.
